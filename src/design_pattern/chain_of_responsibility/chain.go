...
type IApprover interface {
	SetSuccessor(approver *Approver)
...
// 设置后继者：所有具体审批者共用此方法
func (this *Approver) SetSuccessor(approver IApprover) {
...
func newPresident(name string) *Approver {
	return newApprover(name)
}
...
func newCongress(name string) *Approver {
	return newApprover(name)
}
